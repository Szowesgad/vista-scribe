...
-   LLM formatting is optional. To disable: re‑run Install Backend with `FORMAT_ENABLED=0` (or edit the LaunchAgent plist in `~/Library/LaunchAgents/com.vista-scribe.backend.plist`).
...
5.  Save as “Transkrybuj (vista-scribe)”.

Usage: Right‑click any audio file in Finder → Quick Actions → “Transkrybuj (vista-scribe)”. The result is saved next to the file as .transkrypcja.txt, copied to clipboard, and a macOS notification appears.
